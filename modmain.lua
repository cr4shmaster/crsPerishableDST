function crsCustomPerishable(inst)
    inst.CustomUpdate = function(this, dt)
        if this.components.perishable then
            ...
            if not owner and this.components.occupier then
                owner = this.components.occupier:GetOwner()
            end
            if owner then
                ...
                elseif owner:HasTag("cage") and this:HasTag("small_livestock") then
            ...
            modifier = modifier * this.components.perishable.localPerishMultiplyer * TUNING.PERISH_GLOBAL_MULT
